...
this.scanGalleries = function(dirReader) {
    gCurOptGrp = addGallery('User media', 0);
    gGalleryData[gGalleryIndex] = new GalleryData(0);
...
/*
 * chrome.mediaGalleries.onScanProgress.addListener(function(details) { if
 * (details.type == 'finish') { document.getElementById('status').innerText =
 * 'Scan found ' + details.galleryCount + ' galleries'; } else {
 * document.getElementById('status').innerText = 'Scanning: ' + details.type; }
 * if (details.type != 'start') document.getElementById('scan-button').innerHTML =
